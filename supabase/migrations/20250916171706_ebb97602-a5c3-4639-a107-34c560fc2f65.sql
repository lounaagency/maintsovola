-- Migration corrigée pour les cultures restantes et corrections de sécurité
...
-- Jalons pour le Coton (création des jalons)
INSERT INTO jalon_agricole (id_culture, nom_jalon, delai_apres_lancement, action_a_faire, description)
...
-- Jalons et coûts pour Tomates (simplifié)
INSERT INTO jalon_agricole (id_culture, nom_jalon, delai_apres_lancement, action_a_faire, description)
...
-- Jalons pour Carottes (simplifié)
INSERT INTO jalon_agricole (id_culture, nom_jalon, delai_apres_lancement, action_a_faire, description)
...
-- Coûts génériques pour toutes les nouvelles cultures (Tomates et Carottes)
INSERT INTO cout_jalon_reference (id_culture, id_jalon_agricole, type_depense, montant_par_hectare, unite)
SELECT c.id_culture, ja.id_jalon_agricole, 'Main d''œuvre', 
    CASE 
        WHEN ja.nom_jalon LIKE '%Préparation%' THEN 100000
        WHEN ja.nom_jalon LIKE '%Semis%' THEN 60000
        WHEN ja.nom_jalon LIKE '%Repiquage%' THEN 180000
        WHEN ja.nom_jalon LIKE '%Récolte%' THEN 200000
        ELSE 80000
    END, 'Ar/ha'
FROM culture c
JOIN jalon_agricole ja ON c.id_culture = ja.id_culture
WHERE c.nom_culture IN ('Tomates', 'Carottes')
AND NOT EXISTS (SELECT 1 FROM cout_jalon_reference cjr WHERE cjr.id_culture = c.id_culture AND cjr.id_jalon_agricole = ja.id_jalon_agricole);
...
-- Nettoyer les doublons pour "Poids de Mbambara" (CORRECTION DE L'AMBIGUÏTÉ)
WITH doublons AS (
    SELECT ja.id_jalon_agricole, 
           ROW_NUMBER() OVER (PARTITION BY ja.id_culture, ja.nom_jalon ORDER BY ja.id_jalon_agricole) as rn
