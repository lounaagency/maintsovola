...
const TerrainTable: React.FC<TerrainTableProps> = ({
  ...
  const sortedTerrains = [...terrains].sort((a, b) => {
    ...
    if (typeof aValue === 'boolean' && typeof bValue === 'boolean') {
      return sortOptions.direction === 'asc' ? (aValue ? 1 : 0) - (bValue ? 1 : 0) : (bValue ? 1 : 0) - (aValue ? 1 : 0);
    }
    return 0;
  });
  const filteredTerrains = sortedTerrains.filter(terrain => {
    if (searchQuery) {
      ...
      const searchable = [terrain.nom_terrain, terrain.region_name, terrain.district_name, terrain.commune_name, terrain.tantsahaNom, terrain.techniqueNom].filter(Boolean).join(' ').toLowerCase();
  ...
  const handleAssignTechnician = async () => {
    ...
    try {
      const {
        error
      } = await supabase.from('terrain').update({
      ...
      }], "Terrain assigné", `Vous avez été assigné au terrain ${selectedTerrain.nom_terrain}`, "info", "terrain", selectedTerrain.id_terrain);
      await sendNotification(supabase, user.id, [{
        id_utilisateur: selectedTerrain.id_tantsaha || ''
      }], "Technicien assigné", `Un technicien a été assigné à votre terrain ${selectedTerrain.nom_terrain}`, "info", "terrain", selectedTerrain.id_terrain);
      ...
      {isMobile ? <div>
          <div className="flex flex-col gap-4 md:flex-row md:items-center mb-4 justify-between">
            <div className="flex flex-1 items-center gap-2">
              ...
              <Input placeholder="Rechercher un terrain..." value={searchQuery} onChange={handleSearch} className="w-full md:w-[300px]" />
            ...
            {showFilters && <div className="bg-card p-4 rounded-md shadow-sm border mt-2 mb-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <label className="text-sm font-medium">Accès à l'eau</label>
                    <div className="flex items-center space-x-2 mt-1">
                      <Checkbox id="waterAccess" checked={filters.hasWater === true} onCheckedChange={checked => setFilters({
                  ...filters,
                  hasWater: checked ? true : undefined
                })} />
                      <label htmlFor="waterAccess" className="text-sm font-normal leading-none">
                        Avec accès à l'eau
                      </label>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center space-x-2 mt-1">
                      <Checkbox id="roadAccess" checked={filters.hasRoad === true} onCheckedChange={checked => setFilters({
                  ...filters,
                  hasRoad: checked ? true : undefined
                })} />
                      <label htmlFor="roadAccess" className="text-sm font-normal leading-none">
          ...
          {filteredTerrains.length === 0 ? <div className="text-center py-8 border rounded-lg border-dashed">
              {searchQuery || Object.keys(filters).length > 0 ? "Aucun terrain correspondant à vos critères" : "Aucun terrain disponible"}
            </div> : <div className="space-y-4">
              {filteredTerrains.map(terrain => <div key={terrain.id_terrain} className="border rounded-md p-3 bg-white">
                  ...
                  {userRole === 'superviseur' && <div className="mb-3">
                      <p className="text-xs text-muted-foreground mb-1">Technicien</p>
                      {terrain.id_technicien ? <p className="text-sm">{terrain.techniqueNom || 'Non spécifié'}</p> : <Button variant="outline" size="sm" className="w-full" onClick={() => {
              ...
              setIsAssigningTech(true);
                  ...
                  {userRole === 'simple' && type === 'validated' && terrain.techniqueNom && <div className="mb-3">
                      <p className="text-xs text-muted-foreground mb-1">Technicien</p>
                      <p className="text-sm">{terrain.techniqueNom || 'Non assigné'}</p>
                    </div>}
                  
                  {type === 'validated' && <div className="mb-3">
                  ...
                  <div className="flex items-center justify-between flex-wrap gap-2 mt-3">
                    {onViewDetails && <Button variant="outline" size="sm" onClick={() => onViewDetails(terrain)} className="bg-yellow-400 hover:bg-yellow-300">
                        <Eye className="h-4 w-4 mr-1" />
                        
                      </Button>}
                    
                    {onEdit && <Button variant="outline" size="sm" onClick={() => onEdit(terrain)} className="bg-yellow-400 hover:bg-yellow-300">
                        <Edit className="h-4 w-4 mr-1" />
                        
                      </Button>}
                    
                    {type === 'pending' && onValidate && userRole !== 'simple' && <Button variant="outline" size="sm" onClick={() => onValidate(terrain)}>
                        <Check className="h-4 w-4 mr-1" />
                        
                      </Button>}
                    
                    {onDelete && type === 'pending' && <Button variant="outline" size="sm" onClick={() => onDelete(terrain)} className="text-destructive bg-yellow-400 hover:bg-yellow-300">
          ...
          <AlertDialog open={isAssigningTech} onOpenChange={open => {
        setIsAssigningTech(open);
        if (!open) {
          setSelectedTerrain(null);
          setSelectedTechId('');
        }
            ...
            <AlertDialogContent>
              ...
              <div className="py-4">
                <Select value={selectedTechId} onValueChange={setSelectedTechId}>
                  ...
                  <SelectContent>
                    {techniciens.map(tech => <SelectItem key={tech.id_utilisateur} value={tech.id_utilisateur}>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>
                      <div className="flex items-center cursor-pointer" onClick={() => toggleSort('nom_terrain')}>
                        Nom
                        <ArrowUpDown className="ml-2 h-4 w-4" />
                      </div>
                    </TableHead>
                    <TableHead>
                      <div className="flex items-center cursor-pointer" onClick={() => toggleSort('surface_proposee')}>
                        Surface (ha)
                        <ArrowUpDown className="ml-2 h-4 w-4" />
                      </div>
                    </TableHead>
                    <TableHead>
                      <div className="flex items-center cursor-pointer" onClick={() => toggleSort('region_name')}>
                        Localisation
                        <ArrowUpDown className="ml-2 h-4 w-4" />
                      </div>
                    </TableHead>
                    {(userRole === 'superviseur' || userRole === 'technicien') && <TableHead>Propriétaire</TableHead>}
                    {userRole === 'simple' && type === 'validated' && <TableHead>Technicien</TableHead>}
                    {userRole === 'superviseur' && <TableHead>Technicien</TableHead>}
                    <TableHead>
                ...
                <TableBody>
                  {filteredTerrains.map(terrain => <TableRow key={terrain.id_terrain}>
                      ...
                      <TableCell>
                        ...
                        {terrain.surface_validee && terrain.surface_validee !== terrain.surface_proposee && <div className="text-xs text-muted-foreground">
                      ...
                      {(userRole === 'superviseur' || userRole === 'technicien') && <TableCell>{terrain.tantsahaNom || 'Non spécifié'}</TableCell>}
                      {userRole === 'simple' && type === 'validated' && <TableCell>
                          {terrain.techniqueNom !== 'Non assigné' ? <div className="flex items-center">
                              <span>{terrain.techniqueNom}</span>
                              {onContactTechnicien && <Button variant="ghost" size="sm" className="ml-1 p-1 h-auto" onClick={() => onContactTechnicien(terrain)} title="Contacter le technicien">
                                  <MessageSquare className="h-4 w-4 text-muted-foreground" />
                                </Button>}
                            </div> : 'Non assigné'}
                        </TableCell>}
                      {userRole === 'superviseur' && <TableCell>
                          {terrain.id_technicien ? terrain.techniqueNom : <Button variant="outline" size="sm" onClick={() => {
                  setSelectedTerrain(terrain);
                  setIsAssigningTech(true);
                }}>
                              Assigner
                      ...
                      <TableCell>
                        <div className="flex space-x-2">
                          {terrain.acces_eau ? <Badge variant="outline" className="bg-blue-50">Eau</Badge> : null}
                          {terrain.acces_route ? <Badge variant="outline" className="bg-amber-50">Route</Badge> : null}
                        </div>
                      </TableCell>
                      <TableCell className="text-right">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            {onViewDetails && <DropdownMenuItem onClick={() => onViewDetails(terrain)}>
                                <Eye className="mr-2 h-4 w-4" />
                                Voir détails
                              </DropdownMenuItem>}
                            
                            {onEdit && <DropdownMenuItem onClick={() => onEdit(terrain)}>
                                <Edit className="mr-2 h-4 w-4" />
                                Modifier
                              </DropdownMenuItem>}
                            
                            {type === 'pending' && userRole !== 'simple' && onValidate && <DropdownMenuItem onClick={() => onValidate(terrain)}>
                            ...
                            {type === 'pending' && onDelete && <>
                                ...
                                <DropdownMenuItem onClick={() => onDelete(terrain)} className="text-destructive focus:text-destructive">
