...
const TerrainForm: React.FC<TerrainFormProps> = ({ 
  ...
  // Fonction utilitaire pour vérifier s'il existe un terrain qui chevauche le polygone (geom)
  const checkPolygonOverlap = async (geojson: any, idToOmit?: number) => {
    setCheckingOverlap(true);
    let filter = `ST_Intersects(geom, ST_GeomFromGeoJSON('${JSON.stringify(geojson)}')) AND NOT archive`;
    if (idToOmit) {
      filter += ` AND id_terrain != ${idToOmit}`;
    }
    // Utilisation du mode SQL textuel de Supabase pour la géomatique
    const { data, error } = await supabase
      .from("terrain")
      .select(
        "id_terrain, nom_terrain, id_region, id_district, id_commune, surface_proposee, region:region(nom_region), district:district(nom_district), commune:commune(nom_commune), id_tantsaha"
      )
      .filter("archive", "eq", false)
      .or(`geom.ST_Intersects.${JSON.stringify(geojson)}`)
      .not("id_terrain", "eq", idToOmit ?? -1);

    // Si la version de Supabase ne supporte pas or/overwrite syntax, utilise le filtre SQL texte:
    // const { data, error } = await supabase.rpc('find_terrain_intersecting', { search_geojson: geojson, ignore_id: idToOmit ?? null })

    // Filtrage de la réponse : on enlève terrains trop petits (st_intersects renvoie le polygone même pour les frontières touchées)
    setCheckingOverlap(false);
    if (error) {
      console.error("Erreur lors de la vérification du chevauchement des terrains", error);
  ...
  // Soumission du terrain (vérifie recouvrement avant enregistrement)
  const onSubmit = async (formData: TerrainFormData) => {
    ...
    try {
      ...
      if (!tantsahaId) throw new Error("Un agriculteur doit être sélectionné.");
  ...
  return (
    <div className="container max-w-2xl py-6">
      ...
      {/* Alerte de recouvrement si besoin */}
      {overlapTerrains && overlapTerrains.length > 0 && (
        <AlertDialog open={!!overlapTerrains} onOpenChange={(open) => { if (!open) setOverlapTerrains(null); }}>
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Chevauchement détecté avec un terrain existant !</AlertDialogTitle>
              <AlertDialogDescription>
                <div className="space-y-2">
                  <p>Votre polygone recouvre un ou plusieurs terrains existants :</p>
                  <ul className="space-y-2 max-h-56 overflow-y-auto">
                    {overlapTerrains.map(terrain => (
                      <li key={terrain.id_terrain} className="border rounded p-2">
                        <b>{terrain.nom_terrain}</b> <br />
                        {terrain.region?.nom_region || terrain.region_name}, {terrain.district?.nom_district || terrain.district_name}, {terrain.commune?.nom_commune || terrain.commune_name} <br />
                        Surface : {terrain.surface_proposee} ha 
                        <br />
                        Propriétaire: <span className="font-bold">{terrain.id_tantsaha}</span>
                        {/* Optionnel: ici vous pourriez charger le nom de l'utilisateur par id si besoin */}
                      </li>
                    ))}
                  </ul>
                  <p className="text-destructive font-medium mt-2">Veuillez modifier la zone saisie pour qu’elle n’empiète pas sur un autre terrain.</p>
                </div>
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogAction onClick={() => setOverlapTerrains(null)}>OK</AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
      )}
