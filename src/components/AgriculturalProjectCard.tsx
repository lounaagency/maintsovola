  ...
  useEffect(() => {
    const fetchProject = async () => {
      const { data, error } = await supabase
        ...
        .select(`
          *,
          commune:communes(nom, district:districts(nom, region:regions(nom))),
          agriculteur:utilisateurs(nom, pseudo, avatar)
        `)
        .eq("id_projet", projectId) // Utilisation de la clé primaire id_projet
        .single();
      ...
      if (error) {
        ...
        setProject({
          ...
          localisation: {
            ...
            district: data.commune?.district?.nom || "",
            region: data.commune?.district?.region?.nom || "",
          },
        });
      ...
      <AnimatePresence>
        {showComments && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
            <CommentSection postId={project.id_projet} />
          </motion.div>
        )}
      </AnimatePresence>
    </Card>
